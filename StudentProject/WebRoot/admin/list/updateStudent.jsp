...
<html>
	...
	<body>
		...
		<div id="index" style="background-color: royalblue;height:430px;width:900px;float:left;">
			<h1 style="text-align: center;">修改学生</h1>
			<form action="update.do" method="post">
				<fieldset>
					<legend>学生信息修改</legend>
					学号：<input type="text" name="id" pattern="\d*" title="该项仅为数字！" value="${param.id }" readonly /><br /> 
					姓名：<input type="text" name="name" value="${param.name }" required /><br /> 
					性别：<input type="radio" name="sex" value="男" <c:if test="${param.sex == '男'}"> checked </c:if> />男 <input type="radio" name="sex" value="女" <c:if test="${param.sex == '女'}"> checked </c:if> />女<br />
					年龄：<input type="text" name="age" pattern="\d{2}" title="该项为2位数字！" value="${param.age}" required /><br /> 
					电话：<input type="text" name="tel" pattern="\d{11}" title="该项为11位数字！" value="${param.tel }" required /><br /> 
					住址：<input type="text" name="loc" value="${param.loc }" required /><br /> 
